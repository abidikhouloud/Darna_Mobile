 ...
 * @author Khalil
 */
public class AjouterLogement implements Runnable,CommandListener {
    /*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 ...
 */
      ...
      Display disp;
    //Form 1
    Form f1 = new Form("Ajouter Logement");
    ...
    ChoiceGroup  chG  = new ChoiceGroup("region", ChoiceGroup.POPUP,table,null);
      ...
      TextField tfSurfaceCouv = new TextField("Surface couverte ", null, 100, TextField.ANY);        
       TextField tfnbreChambre = new TextField("Nombre chambre ", null, 100, TextField.ANY);   
    ...
    Alert alerta = new Alert("Error", "Sorry", null, AlertType.ERROR);
     ...
     HttpConnection hc;
    DataInputStream dis;
    String url = "http://localhost/parse1/logement/ajoutLogement.php";
    String urllogementlist = "http://localhost/parse1/logement/getXmlPersons_Attributes.php";
    StringBuffer sb1 = new StringBuffer();
    int ch;

    public AjouterLogement(Display disp) {
        this.disp = disp;
         this.startApp();
    }
...
StringBuffer sbu = new StringBuffer();
    ...
    int day = calendar.get(Calendar.DAY_OF_MONTH);
        ...
        try {
            request = (url + "?description=" + tfDescription.getString().trim() + "&date=" +Integer.toString(day).trim()+"-"+Integer.toString(month).trim()+"-"+Integer.toString(year).trim()+"&region=" + table[chG.getSelectedIndex()] +"&prix=" + tfPrix.getString().trim() + "&surface=" + tfSurface.getString().trim() +"&nbrChambre="+tfnbreChambre.getString().trim()+ "&engagement=" + table1[chEngagement.getSelectedIndex()] + "&surfaceCouverte=" + tfSurfaceCouv.getString().trim()).trim();
