 ...
 * @author Khalil
 */
public class MidletListLogements extends MIDlet implements CommandListener, Runnable {
...
Display disp = Display.getDisplay(this);
    ...
    public void afficherLogement()
    {
        ...
        try {//les threads
            ...
            HttpConnection hc = (HttpConnection) Connector.open("http://127.0.0.1/parse1/logement/getXmlPersons_Attributes.php");
    ...
    {
        try {
               HttpConnection hc = (HttpConnection) Connector.open("http://127.0.0.1/parse1/logement/supprimerLogement.php"+"?id="+id);
            ...
            public void modifierLogement() throws IOException
    {
        ...
        String url="http://127.0.0.1/parse1/logement/ModifierMogement.php" ;
        if (logements.length > 0) {
            HttpConnection hc2 = (HttpConnection) Connector.open("http://127.0.0.1/parse1/logement/ModifierLogement.php" +"?id="+id+ "&description=" + tfDescription.getString().trim() + "&region=" + table[chG.getSelectedIndex()].trim() + "&prix=" + tfPrix.getString().trim() + "&surface=" + tfSurface.getString().trim() +  "&surfaceCouverte=" + tfSurfaceCouverte.getString().trim() + "&engagement="+ table1[chEngagement.getSelectedIndex()].trim());
