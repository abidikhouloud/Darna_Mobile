...
public class AjouterTerrain implements Runnable,CommandListener{
    /*
 ...
 */
      ...
      Display disp ;
    //Form 1
    Form f1 = new Form("Ajouter Terrain");
    ...
    Alert alerta = new Alert("Error", "Sorry", null, AlertType.ERROR);
     ...
     HttpConnection hc;
    DataInputStream dis;
    String url = "http://localhost/parse1/terrain/ajoutTerrain.php";
        String urlentrepriselist = "http://localhost/parse1/terrain/getXmlPersons_Attributes.php";
    StringBuffer sb1 = new StringBuffer();
    int ch;

    public AjouterTerrain(Display disp) {
        this.disp = disp;
         this.startApp();
    }
...
StringBuffer sbu = new StringBuffer();
    ...
    int day = calendar.get(Calendar.DAY_OF_MONTH);
        ...
        try {  System.out.println("bonjour");
            request = (url + "?description=" + tfDescription.getString().trim() + "&date=" +Integer.toString(day).trim()+"-"+Integer.toString(month).trim()+"-"+Integer.toString(year).trim()+ "&region=" + table[chG.getSelectedIndex()].trim() + "&prix=" + tfPrix.getString().trim() + "&surface=" + tfSurface.getString().trim() +  "&vocation=" + tfVocation.getString().trim() + "&engagement="+ table1[chEngagement.getSelectedIndex()].trim()).trim();
